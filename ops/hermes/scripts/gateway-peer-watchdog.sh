...
peer_is_healthy() {
  ...
  gateway_status=$(jq -r '.gateway_state // "unknown"' "$gateway_state" 2>/dev/null) || {
    health_reason="gateway_state_invalid"; return 1;
  }
  weixin_status=$(jq -r '.platforms.weixin.state // "unknown"' "$gateway_state" 2>/dev/null) || {
    health_reason="gateway_state_invalid"; return 1;
  }
  pid=$(jq -r '.pid // 0' "$gateway_state" 2>/dev/null) || {
    health_reason="gateway_state_invalid"; return 1;
  }
...
if [ -s "$state_path" ]; then
  previous_failures=$(jq -r '.consecutiveFailures // 0' "$state_path" 2>/dev/null || printf '0')
  previous_alerted=$(jq -r '.alerted // false' "$state_path" 2>/dev/null || printf 'false')
fi
...
send_message() {
  ...
  if ! printf '%s' "$result" | jq -e '.success == true and (.message_id | type == "string") and (.message_id | length > 0)' >/dev/null 2>&1; then
    return 1
  fi
  provider_message_id=$(printf '%s' "$result" | jq -r '.message_id')
  return 0
}
...
write_state() {
  ...
  if [ -s "$state_path" ]; then
    if [ -z "$final_receipt" ]; then final_receipt=$(jq -r '.providerMessageId // ""' "$state_path" 2>/dev/null || true); fi
    if [ -z "$final_kind" ]; then final_kind=$(jq -r '.lastNotificationKind // ""' "$state_path" 2>/dev/null || true); fi
    notified_at=$(jq -r '.lastNotifiedAt // ""' "$state_path" 2>/dev/null || true)
  fi
  ...
  jq -n \
    ...
    --arg receipt "$final_receipt" \
    --arg notification_kind "$final_kind" \
    --arg notified_at "$notified_at" \
    --argjson failures "$failures" \
    --argjson alerted "$alerted" \
    '{peer:$peer,health:$health,consecutiveFailures:$failures,alerted:$alerted,lastReason:$reason,checkedAt:$checked_at,providerMessageId:(if $receipt == "" then null else $receipt end),lastNotificationKind:(if $notification_kind == "" then null else $notification_kind end),lastNotifiedAt:(if $notified_at == "" then null else $notified_at end)}' > "$temporary"
  chmod 600 "$temporary"
