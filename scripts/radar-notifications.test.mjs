...
test('Hermes dispatchers are model-free, one-at-a-time and keep the radar lanes isolated', () => {
  ...
  const transport = read('ops/hermes/market-radar-weixin/transport.py')
  const registration = read('ops/hermes/market-radar-weixin/__init__.py')
  const hermesReadme = read('ops/hermes/README.md')
  ...
  assert.match(transport, /market:quant/)
  assert.match(common, /remember_delivery/)
  assert.match(common, /PRODUCTION_BASE_URL = "https:\/\/xiuqiu-site\.vercel\.app"/)
  assert.match(common, /"Authorization": f"Bearer \{token\}"/)
  assert.match(common, /fcntl\.LOCK_EX/)
  assert.doesNotMatch(common, /127\.0\.0\.1|RADAR_INTERNAL_HMAC|api_base_url|page_base_url/)
  ...
  assert.match(transport, /providerMessageId/)
  assert.match(transport, /idempotency_key=idempotency_key/)
  assert.match(transport, /cooldown_home/)
  assert.match(common, /secondary_profile/)
  assert.match(transport, /recipientAlias/)
  assert.match(common, /await dispatch_once\(spec, chat_id\)/)
  assert.match(common, /print\("\[SILENT\]"\)/)
  assert.match(transport, /await _build_deliveries/)
  assert.match(transport, /logical_delivery_incomplete/)
  assert.match(transport, /startswith\("hermes-weixin-"\)/)
  assert.match(platform, /radar_envelope_mode_not_allowed/)
  assert.match(hermesReadme, /--no-agent --deliver local/)
  assert.match(hermesReadme, /不得使用 `--deliver radar_weixin`/)
  assert.doesNotMatch(`${common}\n${platform}\n${transport}`, /store_pending_delivery|load_pending_delivery|deliveryRef|radar-weixin-pending/)
  ...
  assert.match(market, /claim_path="\/api\/market-radar\/outbox\/claim"/)
  assert.match(market, /ack_path="\/api\/market-radar\/outbox\/ack"/)
  assert.match(learning, /kinds=\("daily",\)/)
  assert.match(learning, /page_prefix="\/radar\/"/)
  assert.match(learning, /claim_path="\/api\/radar\/outbox\/claim"/)
  assert.match(learning, /ack_path="\/api\/radar\/outbox\/ack"/)
  ...
  const postFunction = common.slice(common.indexOf('def post('), common.indexOf('\ndef check_health('))
  assert.doesNotMatch(postFunction, /for attempt|_retry_delay/)
})

test('Hermes radar Python transport keeps body data in CLI memory and preserves partial receipts', () => {
  const script = new URL('./hermes-radar-dispatch.test.py', import.meta.url)
  const output = execFileSync('python3', [script.pathname], { encoding: 'utf8' })
