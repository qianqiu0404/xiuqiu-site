...
test('the public page is static and contains no runtime API dependency', async () => {
  ...
  assert.match(overview, /<h1>先验证，<br \/><span>再判断。<\/span><\/h1>/)
  assert.match(overview, /它不告诉你买什么。/)
  assert.match(styles, /\.trade-radar-hero h1\{[^}]*font-size:clamp\(3rem,5vw,4\.75rem\)/)
  assert.match(styles, /@media\(max-width:760px\)\{[^}]*\.trade-radar-hero\{padding:6\.5rem 0 5\.25rem\}[^}]*\.trade-radar-hero-grid\{grid-template-columns:1fr;gap:3rem\}[^}]*\.trade-radar-hero h1\{font-size:clamp\(2\.35rem,10vw,3rem\)/)
})
