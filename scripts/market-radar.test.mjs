...
test('the overview consumes the public API while dated snapshots remain static and hash-compatible', async () => {
  const { readFile } = await import('node:fs/promises')
  const overview = await readFile(new URL('../src/pages/MarketRadarPage.vue', import.meta.url), 'utf8')
  const card = await readFile(new URL('../src/components/MarketTimelineCard.vue', import.meta.url), 'utf8')
  ...
  assert.match(overview, /fetch\('\/api\/market-radar\/events\?window=24&limit=30'/)
  assert.match(overview, /fetch\('\/api\/market-radar\/summary'/)
  assert.doesNotMatch(detail, /fetch\(|\/api\/market-radar/)
  ...
  assert.match(card, /v-if="item\.priority !== 'P2'"/)
  assert.match(card, /<details v-else class="trade-event-analysis">/)
  ...
  assert.match(styles, /\.trade-radar-page :where\(a, button, summary, \[tabindex='-1'\]\):focus-visible/)
