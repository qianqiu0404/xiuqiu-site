...
test('the public page uses only the price-free market status runtime dependency', async () => {
  ...
  assert.match(overview, /fetch\('\/api\/market-radar\/market-status'\)/)
  assert.match(overview, /:data-event-snapshot-id="latest\?\.snapshotId"/)
  assert.match(overview, /:data-event-snapshot-as-of="latest\?\.asOf"/)
  assert.match(overview, /:data-snapshot-id="marketStatus\?\.snapshotId"/)
  assert.match(overview, /不接账户 · 不自动下单/)
  assert.match(overview, /<h1>交易雷达<\/h1>/)
  assert.match(overview, /事件、影响路径、验证边界。只做研究，不给买卖指令。/)
