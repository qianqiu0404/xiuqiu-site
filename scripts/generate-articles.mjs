...
function parseArticle(fileName) {
  ...
  if (hasSeries !== hasSeriesOrder) {
    throw new Error(`${fileName}: series and seriesOrder must be provided together.`)
  }
  if (hasSeries && (typeof meta.series !== 'string' || !meta.series.trim())) {
    throw new Error(`${fileName}: series must be a non-empty string.`)
  }
  ...
  return {
    ...
    series: hasSeries ? String(meta.series) : undefined,
    seriesOrder: hasSeriesOrder ? Number(meta.seriesOrder) : undefined,
...
articles.forEach(article => {
  ...
  if (article.series && article.seriesOrder) {
    const seriesPosition = `${article.series}:${article.seriesOrder}`
    if (seriesPositions.has(seriesPosition)) throw new Error(`Duplicate article series position: ${seriesPosition}`)
    seriesPositions.add(seriesPosition)
  }
...
rmSync(ARTICLES_OUTPUT_URL, { force: true })
